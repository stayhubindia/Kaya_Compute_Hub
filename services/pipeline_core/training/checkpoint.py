...
class TrainingCheckpointManager:
    ...
    def save_full_checkpoint(
        ...
        self.package_checkpoint_zip(step)
        self._rotate_checkpoints()
        return ckpt_dir

    def package_checkpoint_zip(self, step: int) -> Path:
        """
        Package checkpoint directory into an atomic ZIP archive with manifest and SHA-256 digest.
        Produces:
          checkpoints/checkpoint-step-{step:06d}.zip
          checkpoints/checkpoint-step-{step:06d}.manifest.json
          checkpoints/checkpoint-step-{step:06d}.sha256
        """
        import hashlib
        import zipfile
        import tempfile
        ...
        ckpt_dir = self.get_checkpoint_dir(step)
        zip_name = f"checkpoint-step-{step:06d}.zip"
        manifest_name = f"checkpoint-step-{step:06d}.manifest.json"
        sha_name = f"checkpoint-step-{step:06d}.sha256"

        target_dir = self.output_dir / "checkpoints"
        target_dir.mkdir(parents=True, exist_ok=True)

        zip_path = target_dir / zip_name
        manifest_path = target_dir / manifest_name
        sha_path = target_dir / sha_name

        # Create ZIP in temporary file first (atomic write pattern)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".zip", dir=str(target_dir)) as tmp_zip:
        ...
        try:
            with zipfile.ZipFile(tmp_zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                for file_path in ckpt_dir.rglob("*"):
                    if file_path.is_file():
                        arcname = file_path.relative_to(ckpt_dir)
            ...
            if tmp_zip_path.exists():
                tmp_zip_path.unlink()
            raise RuntimeError(f"Failed to package checkpoint step {step}: {e}") from e
