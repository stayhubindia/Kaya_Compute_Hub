        ...
        if ext == ".pdf":
            ...
            try:
                    ...
                    with open(file_path, "r", encoding="utf-8", errors="replace") as f_in:
                        for line_no, line in enumerate(f_in, 1):
                            ...
                            try:
                                ...
                                logger.warning(f"Malformed JSONL line {line_no} in {file_path}: {line_err}")
                ...
                for idx, data in enumerate(records):
                    ...
                    line_suffix = f"_line{data.get('_line_no', idx+1)}" if ext == ".jsonl" else ""
                    doc_hash = f"{file_hash[:16]}{line_suffix}"
                    meta = IngestionDocumentMetadata(
                        title=data.get("title") or f"{file_path.stem}{line_suffix}",
                    ...
                    sec = Section(
                        section_id=f"{doc_hash[:12]}:sec_0",
                        title="Content",
                        section_type="main",
                        paragraphs=[text_body],
                    )
                    sub_doc = IngestionDocument(
                ...
                # Process primary document and return
                doc = docs[0]
            except Exception as e:
                logger.warning(f"Error parsing JSON/JSONL file {file_path}: {e}")
                return None, []
        else:
            return None, []
